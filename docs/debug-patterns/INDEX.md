...
| `scanner-without-valuer` | query-200 DIFF on a JSONB column, GOT is `{"Data":…,"Valid":true}` wrapper vs bare payload EXP, sibling `json.RawMessage` field matches, nullable wrapper type in `nulltypes/` | Add value-receiver `Value() (driver.Value, error)` to the type (Scanner-only → add Valuer); return inner payload or SQL NULL |
