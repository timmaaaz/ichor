...
func processCommands(args conf.Args, log *logger.Logger, cfg config) error {
	...
	case "seed-frontend":
		if err := commands.SeedFrontend(log, dbConfig); err != nil {
	...
	case "useradd":
		...
		email := args.Num(4)
		password := args.Num(5)
		if err := commands.UserAdd(log, dbConfig, username, firstName, lastName, email, password); err != nil {
	...
	case "validate-configs":
		if err := commands.ValidateConfigs(); err != nil {
