...
func insertSeedData(db *dbtest.Database, ath *auth.Auth) (apitest.SeedData, error) {
	...
	products, err := productbus.TestSeedProducts(ctx, 5, brandIDs, pcIDs, busDomain.InventoryProduct)
	if err != nil {
	...
	metrics, err := metricsbus.TestSeedMetrics(ctx, 20, productIDs, busDomain.Metrics)
	if err != nil {
	...
	for _, ta := range tas {
		...
		if ta.TableName == metricsapi.RouteTable {
