...
func insertSeedData(db *dbtest.Database, ath *auth.Auth) (apitest.SeedData, error) {
	...
	count := 10
	statuses, err := purchaseorderlineitemstatusbus.TestSeedPurchaseOrderLineItemStatuses(ctx, count, busDomain.PurchaseOrderLineItemStatus)
	if err != nil {
		return apitest.SeedData{}, fmt.Errorf("seeding purchase order line item statuses : %w", err)
	}
	...
	for _, ta := range tas {
		// Only update for the purchase_order_line_item_statuses table
		if ta.TableName == purchaseorderlineitemstatusapi.RouteTable {
	...
	return apitest.SeedData{
		...
		PurchaseOrderLineItemStatuses: purchaseorderlineitemstatusapp.ToAppPurchaseOrderLineItemStatuses(statuses),
	}, nil
}
