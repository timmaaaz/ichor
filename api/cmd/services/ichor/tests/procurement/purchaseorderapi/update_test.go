	...
	table := []apitest.Table{
		{
			...
			ExpResp: &purchaseorderapp.PurchaseOrder{
				...
				CurrencyID:              sd.PurchaseOrders[1].CurrencyID,
	...
	table := []apitest.Table{
		{
			...
			GotResp: &errs.Error{},
			ExpResp: errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: procurement.purchase_orders"),
			CmpFunc: func(got, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
