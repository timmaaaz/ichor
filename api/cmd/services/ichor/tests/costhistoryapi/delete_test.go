...
func delete200(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "asadmin",
			URL:        fmt.Sprintf("/v1/finance/costhistory/%s", sd.CostHistory[1].CostHistoryID),
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/finance/costhistory/%s", "not-a-uuid"),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodDelete,
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/finance/costhistory/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodDelete,
