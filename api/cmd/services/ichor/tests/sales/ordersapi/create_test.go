	...
	table := []apitest.Table{
		{
			...
			Input: &ordersapp.NewOrder{
				...
				DueDate:             time.Now().Add(3 * 24 * time.Hour).Format("2006-01-02 15:04:05 -0700 MST"),
			},
...
func create401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			StatusCode: http.StatusUnauthorized,
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.Unauthenticated, "user does not have permission CREATE for table: orders"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
