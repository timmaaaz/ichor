...
func delete200(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		{
			Name:       "asadmin",
			URL:        fmt.Sprintf("/v1/order/line-item-fulfillment-statuses/%s", sd.LineItemFulfillmentStatuses[2].ID),
...
func delete401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		{
			Name:       "emptytoken",
			URL:        fmt.Sprintf("/v1/order/line-item-fulfillment-statuses/%s", sd.LineItemFulfillmentStatuses[0].ID),
			Token:      "&nbsp;",
			Method:     http.MethodDelete,
