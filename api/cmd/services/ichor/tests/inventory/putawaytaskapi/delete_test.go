...
func delete200(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "basic",
			URL:        fmt.Sprintf("/v1/inventory/put-away-tasks/%s", sd.PutAwayTasks[3].ID),
...
func delete401(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission DELETE for table: inventory.put_away_tasks"),
			CmpFunc: func(got, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
