...
func insertSeedData(db *dbtest.Database, ath *auth.Auth) (apitest.SeedData, error) {
	...
	tasks, err := putawaytaskbus.TestSeedPutAwayTasks(ctx, 4, productIDs, locationIDs, createdByIDs, nil, busDomain.PutAwayTask)
	if err != nil {
	...
	for _, ta := range tas {
		if ta.TableName == putawaytaskapi.RouteTable {
