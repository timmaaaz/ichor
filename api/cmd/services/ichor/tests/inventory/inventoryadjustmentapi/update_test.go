...
func update200(sd apitest.SeedData) []apitest.Table {
	...
	return []apitest.Table{
		{
			Name:       "basic",
			URL:        fmt.Sprintf("/v1/inventory/inventory-adjustments/%s", sd.InventoryAdjustments[1].InventoryAdjustmentID),
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/inventory/inventory-adjustments/%s", sd.InventoryAdjustments[0].InventoryAdjustmentID),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
		...
		},
		{
			...
			URL:        fmt.Sprintf("/v1/inventory/inventory-adjustments/%s", "not-a-uuid"),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
			StatusCode: http.StatusBadRequest,
			Input: &inventoryadjustmentapp.UpdateInventoryAdjustment{
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: inventory.inventory_adjustments"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/inventory/inventory-adjustments/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
