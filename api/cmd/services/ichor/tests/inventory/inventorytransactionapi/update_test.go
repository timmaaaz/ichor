...
func update200(sd apitest.SeedData) []apitest.Table {
	...
	return []apitest.Table{
		{
			Name:       "basic",
			URL:        fmt.Sprintf("/v1/inventory/inventory-transactions/%s", sd.InventoryTransactions[0].InventoryTransactionID),
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: inventory.inventory_transactions"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
