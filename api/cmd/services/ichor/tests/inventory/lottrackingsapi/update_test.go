...
func update200(sd apitest.SeedData) []apitest.Table {
	...
	return []apitest.Table{
		{
			Name:       "basic",
			URL:        fmt.Sprintf("/v1/inventory/lot-trackings/%s", sd.LotTrackings[0].LotID),
			...
			Input: &lottrackingsapp.UpdateLotTrackings{
				...
				ReceivedDate:      dbtest.StringPointer(rd.Format(timeutil.FORMAT)),
			...
			ExpResp: &lottrackingsapp.LotTrackings{
				...
				ReceivedDate:      rd.Format(timeutil.FORMAT),
				Quantity:          "15",
				QualityStatus:     "good",
...
func update400(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		...
		},
		{
			...
			URL:        fmt.Sprintf("/v1/inventory/lot-trackings/%s", "not-a-uuid"),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
			StatusCode: http.StatusBadRequest,
			Input: &lottrackingsapp.UpdateLotTrackings{
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: inventory.lot_trackings"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
...
func update404(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "supplier-dne",
			URL:        fmt.Sprintf("/v1/inventory/lot-trackings/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
