	...
	return []apitest.Table{
		...
		},
		{
			...
			Input: &lottrackingsapp.NewLotTrackings{
				...
				ManufactureDate:   md.Format(timeutil.FORMAT),
				ExpirationDate:    ed.Format(timeutil.FORMAT),
				ReceivedDate:      rd.Format(time.RFC1123),
				Quantity:          "15",
				QualityStatus:     "poor",
			},
			GotResp: &errs.Error{},
...
func create401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission CREATE for table: inventory.lot_trackings"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
