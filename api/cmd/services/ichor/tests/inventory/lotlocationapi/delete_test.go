...
func delete200(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "asadmin",
			URL:        fmt.Sprintf("/v1/inventory/lot-locations/%s", sd.LotLocations[1].ID),
...
func delete400(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "malformed-lot-location-id",
...
func delete404(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "lot-location-id-dne",
			URL:        fmt.Sprintf("/v1/inventory/lot-locations/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodDelete,
			StatusCode: http.StatusNotFound,
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.NotFound, "query by id: lot location not found"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
...
func delete401(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission DELETE for table: inventory.lot_locations"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
