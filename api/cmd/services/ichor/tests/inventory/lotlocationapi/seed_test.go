...
func insertSeedData(db *dbtest.Database, ath *auth.Auth) (apitest.SeedData, error) {
	...
	// LOT TRACKING DEPENDENCIES
	suppliers, err := supplierbus.TestSeedSuppliers(ctx, count, contactIDs, busDomain.Supplier)
	if err != nil {
	...
	pc, err := productcategorybus.TestSeedProductCategories(ctx, count, busDomain.ProductCategory)
	if err != nil {
	...
	products, err := productbus.TestSeedProducts(ctx, 15, brandIDs, pcIDs, busDomain.Product)
	if err != nil {
	...
	lotTrackings, err := lottrackingsbus.TestSeedLotTrackings(ctx, 10, supplierProductIDs, busDomain.LotTrackings)
	if err != nil {
		return apitest.SeedData{}, fmt.Errorf("seeding lot trackings : %w", err)
	}

	lotIDs := make(uuid.UUIDs, len(lotTrackings))
	for i, lt := range lotTrackings {
		lotIDs[i] = lt.LotID
	}

	// INVENTORY LOCATION DEPENDENCIES
	...
	zones, err := zonebus.TestSeedZone(ctx, 10, warehouseIDs, busDomain.Zones)
	if err != nil {
	...
	locationIDs := make(uuid.UUIDs, len(inventoryLocations))
	...
	for _, ta := range tas {
		if ta.TableName == lotlocationapi.RouteTable {
	...
	return apitest.SeedData{
		...
		LotLocations:       lotlocationapp.ToAppLotLocations(lotLocations),
	}, nil
}
