...
func update200(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "basic",
			URL:        fmt.Sprintf("/v1/inventory/inventory-items/%s", sd.InventoryItems[1].ID),
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/inventory/inventory-items/%s", sd.InventoryItems[0].ID),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: inventory.inventory_items"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
