...
func insertSeedData(db *dbtest.Database, ath *auth.Auth) (apitest.SeedData, error) {
	...
	contacts, err := contactinfosbus.TestSeedContactInfos(ctx, warehouseCount, strIDs, tzIDs, busDomain.ContactInfos)
	if err != nil {
	...
	// SUPPLIERS + SUPPLIER PRODUCTS + LOTS + SERIALS
	...
	lotTrackingIDs := make(uuid.UUIDs, len(lotTrackings))
	for i, lt := range lotTrackings {
		lotTrackingIDs[i] = lt.LotID
	}

	serialNumbers, err := serialnumberbus.TestSeedSerialNumbers(ctx, 10, lotTrackingIDs, productIDs, inventoryLocationIDs, busDomain.SerialNumber)
	if err != nil {
	...
	for _, ta := range tas {
		if ta.TableName == scanapi.RouteTable {
