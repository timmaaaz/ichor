...
func update200(sd apitest.SeedData) []apitest.Table {
	...
	return []apitest.Table{
		{
			Name:       "basic",
			URL:        fmt.Sprintf("/v1/quality/inspections/%s", sd.Inspections[0].InspectionID),
...
func update400(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
...

		{
			...
			URL:        fmt.Sprintf("/v1/quality/inspections/%s", "not-a-uuid"),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
			StatusCode: http.StatusBadRequest,
			Input: &inspectionapp.UpdateInspection{
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			StatusCode: http.StatusUnauthorized,
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.Unauthenticated, "user does not have permission UPDATE for table: quality_inspections"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
...
func update404(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "supplier-dne",
			URL:        fmt.Sprintf("/v1/quality/inspections/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
