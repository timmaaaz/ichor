...
func update200(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "basic",
			URL:        fmt.Sprintf("/v1/products/product-categories/%s", sd.ProductCategories[1].ID),
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		{
			Name:       "emptytoken",
			URL:        fmt.Sprintf("/v1/products/product-categories/%s", sd.ProductCategories[0].ID),
			Token:      "&nbsp",
			Method:     http.MethodPut,
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: products.product_categories"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/products/product-categories/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
