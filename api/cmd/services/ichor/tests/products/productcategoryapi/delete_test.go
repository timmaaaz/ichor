	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/products/product-categories/%s", "not-a-uuid"),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodDelete,
...
func delete401(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission DELETE for table: products.product_categories"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
