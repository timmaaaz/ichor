...
func insertSeedData(db *dbtest.Database, ath *auth.Auth) (apitest.SeedData, error) {
	...
	pc, err := productcategorybus.TestSeedProductCategories(ctx, 5, busDomain.ProductCategory)
	if err != nil {
	...
	for _, ta := range tas {
		if ta.TableName == productuomapi.RouteTable {
	...
	return apitest.SeedData{
		...
		ProductUOMs:       productuomapp.ToAppProductUOMs(uoms),
	}, nil
}
