...
func update200(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "basic",
			URL:        fmt.Sprintf("/v1/products/brands/%s", sd.Brands[1].ID),
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/products/brands/%s", sd.Brands[0].ID),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
		...
		},
		{
			...
			URL:        fmt.Sprintf("/v1/products/brands/%s", "not-a-uuid"),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
			StatusCode: http.StatusBadRequest,
			Input: &brandapp.UpdateBrand{
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: products.brands"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
...
func update404(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "brand-dne",
			URL:        fmt.Sprintf("/v1/products/brands/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
