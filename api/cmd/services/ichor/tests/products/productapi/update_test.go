	...
	return []apitest.Table{
		{
			...
			Input: &productapp.UpdateProduct{
				...
				UpcCode:              dbtest.StringPointer("updated-upc-code"),
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: products.products"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
