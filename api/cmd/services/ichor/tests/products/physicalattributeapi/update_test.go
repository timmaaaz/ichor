...
func update200(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "basic",
			URL:        fmt.Sprintf("/v1/products/physical-attributes/%s", sd.PhysicalAttributes[1].ID),
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/products/physical-attributes/%s", sd.PhysicalAttributes[0].ID),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
		...
		},
		{
			...
			URL:        fmt.Sprintf("/v1/products/physical-attributes/%s", "not-a-uuid"),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
			StatusCode: http.StatusBadRequest,
			Input: &physicalattributeapp.UpdatePhysicalAttribute{
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: products.physical_attributes"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
...
func update404(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "attribute-dne",
			URL:        fmt.Sprintf("/v1/products/physical-attributes/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
