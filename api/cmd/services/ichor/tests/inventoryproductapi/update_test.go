	...
	return []apitest.Table{
		{
			...
			ExpResp: &productapp.Product{
				ProductID:            sd.InventoryProducts[1].ProductID,
				Name:                 "Test Product",
				SKU:                  "sku123",
				...
				UnitsPerCase:         "20",
				CreatedDate:          sd.InventoryProducts[1].CreatedDate,
			},
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/inventory/core/products/%s", sd.InventoryProducts[0].ProductID),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodPut,
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			StatusCode: http.StatusUnauthorized,
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.Unauthenticated, "user does not have permission UPDATE for table: inventory_products"),
			CmpFunc: func(got any, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
