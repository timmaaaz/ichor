...
func delete200(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "asadmin",
			URL:        fmt.Sprintf("/v1/movement/inventoryadjustment/%s", sd.InventoryAdjustments[1].InventoryAdjustmentID),
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/movement/inventoryadjustment/%s", "not-a-uuid"),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodDelete,
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/movement/inventoryadjustment/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodDelete,
