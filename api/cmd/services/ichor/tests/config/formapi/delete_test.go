...
func delete200(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		{
			Name:       "basic",
			URL:        "/v1/config/forms/" + sd.Forms[1].ID,
...
func delete400(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		{
			Name:       "invalid-id",
			URL:        "/v1/config/forms/not-a-uuid",
			Token:      sd.Admins[0].Token,
			Method:     http.MethodDelete,
...
func delete401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission DELETE for table: config.forms"),
			CmpFunc: func(got, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
