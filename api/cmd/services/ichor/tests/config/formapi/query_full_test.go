...
func queryFullByName200(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		{
			Name:       "basic",
			URL:        "/v1/config/forms/name/" + sd.Forms[0].Name + "/full",
			Token:      sd.Users[0].Token,
			StatusCode: 200,
			Method:     "GET",
			GotResp:    &formapp.FormFull{},
...
func queryFullByName401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		{
			Name:       "unauthorized",
			URL:        "/v1/config/forms/name/" + sd.Forms[0].Name + "/full",
...
func queryFullByName404(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		{
			Name:       "not-found",
			URL:        "/v1/config/forms/name/NonExistentFormName/full",
			Token:      sd.Users[0].Token,
			StatusCode: 404,
			Method:     "GET",
			GotResp:    &errs.Error{},
