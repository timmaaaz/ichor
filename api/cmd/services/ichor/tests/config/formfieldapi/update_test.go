	...
	table := []apitest.Table{
		{
			...
			ExpResp: &formfieldapp.FormField{
				...
				EntitySchema: sd.FormFields[0].EntitySchema,
				EntityTable:  sd.FormFields[0].EntityTable,
...
func update401(sd apitest.SeedData) []apitest.Table {
	table := []apitest.Table{
		...
		},
		{
			...
			GotResp:    &errs.Error{},
			ExpResp:    errs.Newf(errs.PermissionDenied, "user does not have permission UPDATE for table: config.form_fields"),
			CmpFunc: func(got, exp any) string {
				return cmp.Diff(got, exp)
			},
		},
