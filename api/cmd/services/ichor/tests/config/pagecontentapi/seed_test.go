...
func insertSeedData(db *dbtest.Database, ath *auth.Auth) (apitest.SeedData, error) {
	...
	// Seed a page config first
	configs, err := pageconfigbus.TestSeedPageConfigs(ctx, 1, busDomain.PageConfig)
	if err != nil {
		return apitest.SeedData{}, fmt.Errorf("seeding page configs : %w", err)
	}

	// Seed page contents for the first page config
	contents, err := pagecontentbus.TestSeedPageContents(ctx, 10, configs[0].ID, busDomain.PageContent)
	if err != nil {
		return apitest.SeedData{}, fmt.Errorf("seeding page contents : %w", err)
	}
	...
	for _, ta := range tas {
		// Only update for the page_content table
		if ta.TableName == pagecontentapi.RouteTable {
	...
	return apitest.SeedData{
		...
		PageContents: pagecontentapp.ToAppPageContents(contents),
	}, nil
}
