...
func delete200(sd apitest.SeedData) []apitest.Table {
	return []apitest.Table{
		{
			Name:       "asadmin",
			URL:        fmt.Sprintf("/v1/warehouses/inventorylocations/%s", sd.InventoryLocations[1].LocationID),
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/warehouses/inventorylocations/%s", "not-a-uuid"),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodDelete,
	...
	return []apitest.Table{
		{
			...
			URL:        fmt.Sprintf("/v1/warehouses/inventorylocations/%s", uuid.NewString()),
			Token:      sd.Admins[0].Token,
			Method:     http.MethodDelete,
