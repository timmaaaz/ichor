...
func (a add) Add(app *web.App, cfg mux.Config) {
	...
	productUOMBus := productuombus.NewBusiness(cfg.Log, productuomdb.NewStore(cfg.Log, cfg.DB))
	...
	if cfg.TemporalClient != nil {
		...
		cfg.Log.Info(context.Background(),
			"temporal: disabled (no client provided)")
	}
	...
	currencyapi.Routes(app, currencyapi.Config{
		Log:            cfg.Log,
	...
	formDataRegistry, err := buildFormDataRegistry(
		...
		currencyapp.NewApp(currencyBus),
