...
- **Two-half architecture (static prevention + runtime backstop) — BOTH halves gate cascade-enablement** (decided 2026-06-10): the cascade gate does NOT open until *both* the runtime visited-set guard AND the static prevention layer are built, tested, and verified — no "armed but unguarded" window, because this is the most critical part of the app. Build order is phased (runtime guard → static, cheapest-first: self-loop block + auto-match warning reuse `cascade.go`'s edge logic, value-aware SCC detector last), each phase planned/executed/tested, but enablement waits for both. **Scope reality:** static can never be 100% (7 handlers irreducibly dynamic — Probe B), so "both ship" = static prevents what it can *prove* + runtime is the universal backstop for the indeterminate band. Grounding: runtime mandatory + cheap (Probe B/D); static substrate ~60% exists (Probe C/E). (§2, extends §0)
- **Policy A — forbid loops by default** (decided 2026-06-10): block *provable* cycles at authoring; warn (don't block) on indeterminate/dynamic; runtime refuses re-entry of a `(rule,entity)` already in the chain. Chosen over B (exit-clause): an arbitrary exit predicate is unsound (halting problem), and Probe A found 0 existing loops + no intentional-loop case (zero migration). (§7)
- **Defer intentional-loop support** (decided 2026-06-10): build forbid-by-default now; do NOT build exit-clause authoring / bounded-loop semantics in v1. Maintainer rationale: intentional loops ARE expected later, and exit clauses belong to that future work. (§8)
- **Runtime mechanism = visited-set of `(ruleID, entityID)`, pure forbid** (decided 2026-06-10): carry the set of pairs fired in this cascade chain on `WorkflowInput.TriggerData` (Continue-As-New-safe — Probe D); check in the matched-rule loop (`trigger.go:128`) before `ExecuteWorkflow`; re-entry of an existing `(R,E)` → stop. Chosen over a bare depth counter (crude in both directions). No `maxReEntries` knob (deferred with intentional-loop support). Depth-cap backstop HELD — add only if a bulk multi-row-write / fan-out handler is found; none confirmed (`update_field`/`create_entity` are single-row). (§7)

- **Static cheap tier = single-rule auto-match self-loop hard-block ONLY** (decided 2026-06-10): block a rule whose trigger is auto-match (`on_update`/no-conditions) on entity E *and* that writes E (reuses `cascade.go` edge logic minus the `:161` self-skip, wired into `prepareRequest`). Gated to auto-match — a conditioned `changed_to` self-reference is legitimate and must NOT be blocked. **DROPPED** §4b's broad "auto-match rule's entity written by any rule" warning: it flags one-way *cascades*, not loops (e.g. DW1 LowStockAlert flagged by rule #5's `inventory_items` write although DW1 never writes back) → noisy on the most-common shape, and redundant once the value-aware detector ships (both halves precede the gate — Item 1). (§4b)
- **Static detector = ONE inter-rule graph + Tarjan SCC, value-aware** (confirmed 2026-06-10): build ALL active rules into a single graph (edge X→Y iff X produces a value satisfying Y's trigger), run SCC/Tarjan (catches `A→B→C→A`), apply §4a re-armability per cycle. Buildable in ~1 query, runs at save + activation against the active set (Probe E). **Three-tier output:** ERROR/block = provable re-arming loop (self-loop or proven SCC cycle); WARN/don't-block = indeterminate cycle (dynamic/templated values — can't prove); INFO/neutral datapoint = any cross-rule cascade edge ("this triggers Workflow X" — awareness, NOT a prohibition; this is the salvaged cascade-map). (§3/§4a)

- **Manifest = single source of truth (Option b)** (decided 2026-06-10): authoritative for static edges + editor cue + **M1 synthetic emit** (drift impossible by construction where the handler owns the write); for B/M2 bus emits a **consistency test** asserts declared == fired. (§6)
- **Write-path core** (decided 2026-06-10): the **protected-list** (block generic writes on the ~30 guarded `(entity,field)` pairs, route them to typed action-verbs) is the must-do validation fix; M1 generic-write cascade = **synthesize** now; **bus-routing / Option B = a per-entity consolidation FOLLOW-UP, not a prereq** (does NOT dissolve the protected-list — domain trace Verdict a). See WRITE_PATH.md + FOLLOW_UP.md.
- **Activation-check scope = active-only** (decided 2026-06-10): run the inter-rule cycle check against the active rule set, enforced at BOTH `prepareRequest` (`workflowsaveapp.go:62`) and `ActivateRule`/`toggleActive` (`workflowbus.go:571`/`ruleapi.go:272`); saving an inactive rule skips it; all-rules stays a one-line future toggle. (§3)
- **Token-threading = `WorkflowInput.TriggerData` + chokepoint** (decided 2026-06-10): the visited-set rides `TriggerData` (CAN-safe), stamped onto the activity ctx once in `activities.go` and read once in `DelegateHandler.handleEvent` before the `:86` goroutine — avoiding the 205-site `DelegateEventParams` change. (§5)
- **Manifest value-extension shape** (decided 2026-06-10): extend `EntityModification` with produced value + operator where statically knowable (4 enum-const handlers; ~4 config-literal via a no-`{{}}` check); dynamic/templated → mark **indeterminate** → conservative edge + defer to runtime guard. (§4)
- **Reliability/ordering = best-effort for v1** (decided 2026-06-10): cascades stay async/best-effort (matches today; the guard doesn't change it). Transactional-outbox / read-after-commit + the FormData tx bug → FOLLOW_UP.md (only load-bearing if Option B is built). (§9)
- **PR #176 = superseded** (decided 2026-06-10): this branch's docs + the real fix replace #176's standalone limitation doc; salvage its accurate `update_field` limitation prose into the eventual user docs; do NOT merge the broken `allocation_results` workaround. (INVESTIGATION §9)
- **Cascade scope — which handlers fire events** (decided 2026-06-10): cascade ONLY the four *advertised domain-state* handlers — `update_field`/`create_entity`/`transition_status` (M1 **synthesize**) + `allocate`/`reserve`→`allocation_results` (M2 `delegate.Call` on `CreateAllocationResult` + fix its unqualified entity name). Keep the three non-advertising **sinks SILENT**: `log_audit_entry` (audit sink — would be audit→audit noise), `create_alert`, `seek_approval` (the meaningful approval event already cascades via `Resolve`). Principle: *fire == advertised domain change.* **Consequence: M2 shrinks to the `allocation_results` fix only** — drop the `alertbus` delegate-field wiring and the `approvalrequestbus.Create` change. Alert/approval-creation cascade → revisit on concrete use case (FOLLOW_UP §8). (INVESTIGATION §9 Q1)
- **Guarded fields with no typed action = Option A (block-now, build-on-demand)** (decided 2026-06-10): block generic writes on all ~30 guarded `(entity,field)` pairs; route to the existing typed action-verb where one exists; where none exists, the field is blocked-with-no-alternative (a safe, visible gap) until a typed action is built on demand. `transition_status` is itself a *generic* handler subject to the block (safe only for non-invariant status). Full enumeration + the four dispositions + the missing-typed-action build list → **FOLLOW_UP.md §9**. (WRITE_PATH §1)
- **Observability = existing logs in v1; rich tracing is a separate track** (decided 2026-06-10): v1 relies on the existing swallowed-delegate-error log line — no new observability in the core. Proper cascade observability (extend the HTTP **OpenTelemetry** pattern to the cascade: inject/extract trace context on the event payload, one span per hop; + optional dropped-cascade metric) is its own follow-up (FOLLOW_UP §10), paired with F2. **Core hook:** build the P1 lineage carrier as a small *extensible* struct on the event payload (visited-set now; room for `traceparent`/correlation later) so the observability track needs no re-plumbing.
- **Protected-list sourcing = domain-declared via registry** (decided 2026-06-10): protected-ness is declared on the domain model (a `protected` struct tag, co-located with the field like `validate` tags), collected at startup into a workflow-readable registry — the `delegate.Register` pattern (domain owns it, `all.go` wires it, the SDK consumes it). Full list = union of (1) fields a typed action claims (from its `GetEntityModifications` manifest — auto) + (2) domain-model-declared protected fields with no typed action yet. **No central hand-maintained list** (it would drift); the P0 consistency test backstops both sources. Exact tag-vs-method form = P3 detail. (idiomatic/boring registry pattern; supersedes the earlier "static map" / "manual supplement" ideas)

**`[LEANING]`** — none.

**`[OPEN]`** — none. Downstream/deferred tracks live in **[`FOLLOW_UP.md`](./FOLLOW_UP.md)** (Option B bus-routing consolidation, FormData tx bug, reliability hardening, PR #176 doc salvage, frontend picker UX, stale arch-doc cleanup).
...
## 12. The write-path / validation dimension → WRITE_PATH.md

A second dimension surfaced while deciding the M1 fix: the generic raw-SQL actions skip business-layer **validation/invariants**, not just delegate events (INVESTIGATION §5). This is tracked separately in **[`WRITE_PATH.md`](./WRITE_PATH.md)** and is **orthogonal to the loop guard** (Items 1–4 stand regardless). Leanings there (correctness-weighted): the **protected-list** (route the ~30 guarded `(entity,field)` pairs to typed action-verbs) is the must-do core fix; **bus-routing via the FormData dispatcher (Option B) is a per-entity consolidation FOLLOW-UP**, not a prereq, and the domain trace showed it does NOT dissolve the protected-list (`bus.Update` bypasses the action-verb guards). Core plan = loop guard + M2 + M1-synthesize + protected-list; B = later consolidation.
