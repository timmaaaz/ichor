# Workflow Temporal Implementation Next Phase Command

Execute the next pending phase in the Workflow Temporal Implementation plan, or continue the current in-progress phase.
...
3. Verify phase documentation exists (e.g., `.claude/plans/WORKFLOW_TEMPORAL_PLAN/phases/PHASE_1_*.md`)
...
Load the corresponding phase file from `.claude/plans/WORKFLOW_TEMPORAL_PLAN/phases/PHASE_N_*.md`

If phase documentation doesn't exist, suggest running `/workflow-temporal-build-phase` first.
...
- Ensure determinism for Temporal workflows
- Use `make dev-bounce` to verify local deployment

### Infrastructure Phases

- Verify K8s manifests with kustomize build
- Test with `make dev-up` and `make dev-apply`
- Check pod status with `make dev-status`

### Testing Phases

- Write tests as you create components
- Aim for meaningful coverage
- Test critical paths thoroughly (especially determinism!)
- Document test scenarios

## Example Usage

User runs: `/workflow-temporal-next`
...
14. "Ready for Phase 3. Run `/workflow-temporal-next` again to continue."
