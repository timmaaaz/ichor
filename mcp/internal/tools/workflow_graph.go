...
// pathStep represents one node in a traced path through the workflow.
...
// pathResult holds the output of a path trace.
type pathResult struct {
	StartingFrom   string     `json:"starting_from"`
	OutputFollowed string     `json:"output_followed,omitempty"`
	Path           []pathStep `json:"path"`
	TextOutline    string     `json:"text_outline"`
}

// tracePath walks the subtree from the given start action IDs, collecting
// structured path steps and a text outline.
func (g *workflowGraph) tracePath(startIDs []string) pathResult {
	var steps []pathStep
	var lines []string
	visited := make(map[string]bool)

	for _, startID := range startIDs {
		g.walkPathNode(startID, 0, "", visited, &steps, &lines)
	}

	return pathResult{
		Path:        steps,
		TextOutline: strings.Join(lines, "\n"),
	}
}

// walkPathNode recursively traverses the graph collecting pathStep entries
// and text outline lines (reusing the walkNode formatting style).
func (g *workflowGraph) walkPathNode(nodeID string, depth int, edgeLabel string, visited map[string]bool, steps *[]pathStep, lines *[]string) {
	action := g.byID[nodeID]
	if action == nil {
		return
	}
	...
	*steps = append(*steps, pathStep{
	...
	// Build text line (mirrors walkNode formatting).
	prefix := strings.Repeat("  ", depth)
	label := action.Name
	if action.ActionType != "" {
		label += " (" + action.ActionType + ")"
	...
	*lines = append(*lines, prefix+label)
	...
	if len(outgoing) == 1 {
		g.walkPathNode(outgoing[0].TargetActionID, depth, formatEdgeLabel(outgoing[0]), visited, steps, lines)
		return
	}

	for _, edge := range outgoing {
		g.walkPathNode(edge.TargetActionID, depth+1, formatEdgeLabel(edge), visited, steps, lines)
	}
}
