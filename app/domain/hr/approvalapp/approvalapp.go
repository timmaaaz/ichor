...
func NewApp(approvalstatusBus *approvalbus.Business) *App {
	return &App{
...
func NewAppWithAuth(approvalstatusBus *approvalbus.Business, ath *auth.Auth) *App {
	return &App{
		auth:                  ath,
		userapprovalstatusbus: approvalstatusBus,
	}
...
func (a *App) NewWithTx(tx sqldb.CommitRollbacker) (*App, error) {
	userapprovalstatusbusTx, err := a.userapprovalstatusbus.NewWithTx(tx)
	if err != nil {
		return nil, err
	}
	nb := *a
	nb.userapprovalstatusbus = userapprovalstatusbusTx
	return &nb, nil
}
...
func (a *App) Update(ctx context.Context, app UpdateUserApprovalStatus, id uuid.UUID) (UserApprovalStatus, error) {
	...
	if err != nil {
		return UserApprovalStatus{}, errs.New(errs.NotFound, approvalbus.ErrNotFound)
	}

	updated, err := a.userapprovalstatusbus.Update(ctx, as, uas)
	if err != nil {
		if errors.Is(err, approvalbus.ErrNotFound) {
...
func (a *App) Delete(ctx context.Context, id uuid.UUID) error {
	as, err := a.userapprovalstatusbus.QueryByID(ctx, id)
	if err != nil {
		return errs.New(errs.NotFound, approvalbus.ErrNotFound)
	}
