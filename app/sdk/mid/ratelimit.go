...
//
//	cidrs := appmid.ParseTrustedCIDRs(cfg.TrustedProxyCIDRs) // e.g. "10.0.0.0/8"
//	httpmid.RateLimit(limiter, appmid.TrustedProxyExtractor(cidrs), 12)
...
//
// A cleanup goroutine runs in the background for the lifetime of the process.
// This matches the existing pattern for long-running goroutines in this service
// (RabbitMQ consumer, WebSocket hub). It is not necessary to stop it explicitly
...
// This prevents the limiters map from growing without bound when many distinct
// IPs make occasional requests (e.g. scanners hitting the auth endpoint once).
