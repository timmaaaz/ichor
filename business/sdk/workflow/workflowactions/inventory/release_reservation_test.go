...
func Test_ReleaseReservation(t *testing.T) {
	db := dbtest.NewDatabase(t, "Test_ReleaseReservation")

	sd, err := insertReleaseReservationSeedData(db.BusDomain)
	if err != nil {
	...
	sd.Handler = inventory.NewReleaseReservationHandler(log, db.DB, db.BusDomain.InventoryItem)

	unitest.Run(t, releaseReservationTests(db.BusDomain, db.DB, sd), "releaseReservation")
}

// =============================================================================

type releaseReservationSeedData struct {
	unitest.SeedData
	Handler            *inventory.ReleaseReservationHandler
...
func insertReleaseReservationSeedData(busDomain dbtest.BusDomain) (releaseReservationSeedData, error) {
	ctx := context.Background()

	admins, err := userbus.TestSeedUsersWithNoFKs(ctx, 1, userbus.Roles.Admin, busDomain.User)
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("seeding user : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("querying regions : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("seeding cities : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("seeding streets : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("querying timezones : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("seeding contact info : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("seeding brand : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("seeding product category : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("seeding product : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("seeding warehouses : %w", err)
	}
	...
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("seeding inventory items : %w", err)
	}

	// Pre-reserve 20 units on the first item for release tests.
	reservedQty := 20
	updatedItem, err := busDomain.InventoryItem.Update(ctx, inventoryItems[0], inventoryitembus.UpdateInventoryItem{
		ReservedQuantity: &reservedQty,
	})
	if err != nil {
		return releaseReservationSeedData{}, fmt.Errorf("pre-reserving inventory : %w", err)
	}
	...
	execContext := workflow.ActionExecutionContext{
		...
		UserID:        adminIDs[0],
		RuleID:        &ruleID,
		RuleName:      "Test Release Reservation Rule",
		ExecutionID:   uuid.New(),
		Timestamp:     time.Now().UTC(),
		TriggerSource: workflow.TriggerSourceAutomation,
	}

	return releaseReservationSeedData{
...
func releaseReservationTests(busDomain dbtest.BusDomain, db *sqlx.DB, sd releaseReservationSeedData) []unitest.Table {
	return []unitest.Table{
		validateReleaseReservationConfig(sd),
		executeBasicRelease(busDomain, sd),
		executeReleaseNegativeGuard(sd),
	}
}

func validateReleaseReservationConfig(sd releaseReservationSeedData) unitest.Table {
	return unitest.Table{
...
func executeBasicRelease(busDomain dbtest.BusDomain, sd releaseReservationSeedData) unitest.Table {
	if len(sd.Products) == 0 || len(sd.InventoryItems) == 0 {
		return unitest.Table{
			Name:    "execute_basic_release_skip",
			ExpResp: "skipped",
	...
	return unitest.Table{
		Name:    "execute_basic_release",
		ExpResp: 10, // New reserved qty after releasing 10 of 20.
		ExcFunc: func(ctx context.Context) any {
			item := sd.InventoryItems[0]

			config := inventory.ReleaseReservationConfig{
...
func executeReleaseNegativeGuard(sd releaseReservationSeedData) unitest.Table {
	if len(sd.Products) < 2 || len(sd.InventoryItems) < 2 {
		return unitest.Table{
			Name:    "execute_negative_guard_skip",
			ExpResp: "skipped",
			ExcFunc: func(ctx context.Context) any { return "skipped" },
			CmpFunc: func(got any, exp any) string { return "" },
		}
	...
	return unitest.Table{
		Name:    "execute_negative_guard",
		ExpResp: true,
		ExcFunc: func(ctx context.Context) any {
			// Second item has reserved_quantity=0, try to release 50 -> should error.
