...
func (b *Business) Create(ctx context.Context, no NewOffice) (Office, error) {
	...
	return outbox.WriteAtomic(ctx, b.outbox, b, (*Business).NewWithTx,
		func(ctx context.Context, b *Business) (Office, error) {
			o := Office{
				ID:       uuid.New(),
				Name:     no.Name,
			...
			evtData := ActionCreatedData(o)
			if err := b.outbox.Emit(ctx, evtData); err != nil {
				return Office{}, fmt.Errorf("emit cascade event: %w", err)
			}
...
func (b *Business) Update(ctx context.Context, o Office, uo UpdateOffice) (Office, error) {
	...
	return outbox.WriteAtomic(ctx, b.outbox, b, (*Business).NewWithTx,
		func(ctx context.Context, b *Business) (Office, error) {
			...
			// Fire delegate event for workflow automation
			evtData := ActionUpdatedData(before, o)
			if err := b.outbox.Emit(ctx, evtData); err != nil {
				return Office{}, fmt.Errorf("emit cascade event: %w", err)
			}
