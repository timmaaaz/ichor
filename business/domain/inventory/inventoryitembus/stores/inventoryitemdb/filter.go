...
// applyFilterAliased is identical to applyFilter but prefixes all column references
// with "ii." for use in queries that JOIN other tables (avoids ambiguity).
func applyFilterAliased(filter inventoryitembus.QueryFilter, data map[string]interface{}, buf *bytes.Buffer) {
	var wc []string

	if filter.ID != nil {
		data["id"] = *filter.ID
		wc = append(wc, "ii.id = :id")
	}

	if filter.ProductID != nil {
		data["product_id"] = *filter.ProductID
		wc = append(wc, "ii.product_id = :product_id")
	}

	if filter.LocationID != nil {
		data["location_id"] = *filter.LocationID
		wc = append(wc, "ii.location_id = :location_id")
	}
	if filter.ReservedQuantity != nil {
		data["reserved_quantity"] = *filter.ReservedQuantity
		wc = append(wc, "ii.reserved_quantity = :reserved_quantity")
	}

	if filter.AllocatedQuantity != nil {
		data["allocated_quantity"] = *filter.AllocatedQuantity
		wc = append(wc, "ii.allocated_quantity = :allocated_quantity")
	}

	if filter.MinimumStock != nil {
		data["minimum_stock"] = *filter.MinimumStock
		wc = append(wc, "ii.minimum_stock = :minimum_stock")
	}

	if filter.MaximumStock != nil {
		data["maximum_stock"] = *filter.MaximumStock
		wc = append(wc, "ii.maximum_stock = :maximum_stock")
	}

	if filter.ReorderPoint != nil {
		data["reorder_point"] = *filter.ReorderPoint
		wc = append(wc, "ii.reorder_point = :reorder_point")
	}

	if filter.EconomicOrderQuantity != nil {
		data["economic_order_quantity"] = *filter.EconomicOrderQuantity
		wc = append(wc, "ii.economic_order_quantity = :economic_order_quantity")
	}

	if filter.SafetyStock != nil {
		data["safety_stock"] = *filter.SafetyStock
		wc = append(wc, "ii.safety_stock = :safety_stock")
	}

	if filter.AvgDailyUsage != nil {
		data["avg_daily_usage"] = *filter.AvgDailyUsage
		wc = append(wc, "ii.avg_daily_usage = :avg_daily_usage")
	}

	if filter.Quantity != nil {
		data["quantity"] = *filter.Quantity
		wc = append(wc, "ii.quantity = :quantity")
	}

	if filter.CreatedDate != nil {
		data["created_date"] = *filter.CreatedDate
		wc = append(wc, "ii.created_date = :created_date")
	}

	if filter.UpdatedDate != nil {
		data["updated_date"] = *filter.UpdatedDate
		wc = append(wc, "ii.updated_date = :updated_date")
	}
