...
func insertSeedData(busDomain dbtest.BusDomain) (unitest.SeedData, error) {
	...
	inventoryItems, err := inventoryitembus.TestSeedInventoryItems(ctx, 30, inventoryLocationsIDs, productIDs, busDomain.InventoryProduct)
	if err != nil {
	...
	return []unitest.Table{
		{
			...
			ExcFunc: func(ctx context.Context) any {
				got, err := busDomain.InventoryProduct.Query(ctx, inventoryitembus.QueryFilter{}, inventoryitembus.DefaultOrderBy, page.MustParse("1", "5"))
				if err != nil {
					return err
				}
				return got
	...
	return []unitest.Table{
		{
			...
			ExcFunc: func(ctx context.Context) any {
				got, err := busDomain.InventoryProduct.Create(ctx, inventoryitembus.NewInventoryItem{
					LocationID:            sd.InventoryLocations[0].LocationID,
					ProductID:             sd.Products[0].ProductID,
					Quantity:              10,
					ReservedQuantity:      15,
	...
	return []unitest.Table{
		{
			...
			ExcFunc: func(ctx context.Context) any {
				got, err := busDomain.InventoryProduct.Update(ctx, sd.InventoryItems[0], inventoryitembus.UpdateInventoryItem{
					LocationID:            &sd.InventoryLocations[0].LocationID,
					ProductID:             &sd.Products[0].ProductID,
	...
	return []unitest.Table{
		{
			Name: "Delete",
			ExcFunc: func(ctx context.Context) any {
				return busDomain.InventoryProduct.Delete(ctx, sd.InventoryItems[0])
			},
			CmpFunc: func(got, exp any) string {
