...
func (s *Store) NewWithTx(tx sqldb.CommitRollbacker) (userapprovalstatusbus.Storer, error) {
...
func (s *Store) Create(ctx context.Context, as userapprovalstatusbus.UserApprovalStatus) error {
	const q = `
	...
	if err := sqldb.NamedExecContext(ctx, s.log, s.db, q, toDBUserApprovalStatus(as)); err != nil {
		if errors.Is(err, sqldb.ErrDBDuplicatedEntry) {
			return fmt.Errorf("namedexeccontext: %w", userapprovalstatusbus.ErrUniqueEntry)
		}
...
func (s *Store) Update(ctx context.Context, as userapprovalstatusbus.UserApprovalStatus) error {
	const q = `
...
func (s *Store) Delete(ctx context.Context, as userapprovalstatusbus.UserApprovalStatus) error {
	const q = `
	DELETE FROM
...
func (s *Store) Query(ctx context.Context, filter userapprovalstatusbus.QueryFilter, orderBy order.By, page page.Page) ([]userapprovalstatusbus.UserApprovalStatus, error) {
	data := map[string]any{
...
func (s *Store) Count(ctx context.Context, filter userapprovalstatusbus.QueryFilter) (int, error) {
	data := map[string]any{}
...
func (s *Store) QueryByID(ctx context.Context, aprvlStatusID uuid.UUID) (userapprovalstatusbus.UserApprovalStatus, error) {
	data := struct {
	...
	if err := sqldb.NamedQueryStruct(ctx, s.log, s.db, q, data, &userApprovalStatus); err != nil {
		if errors.Is(err, sqldb.ErrDBNotFound) {
			return userapprovalstatusbus.UserApprovalStatus{}, fmt.Errorf("db: %w", userapprovalstatusbus.ErrNotFound)
		}
		return userapprovalstatusbus.UserApprovalStatus{}, fmt.Errorf("namedquerystruct: %w", err)
	}
