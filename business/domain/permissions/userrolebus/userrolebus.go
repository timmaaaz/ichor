...
var (
	...
	ErrUnique                = errors.New("user role is not unique")
...
func (b *Business) Update(ctx context.Context, ur UserRole, uur UpdateUserRole) (UserRole, error) {
	...
	if err := b.del.Call(ctx, ActionUpdatedData(ur)); err != nil {
		return UserRole{}, fmt.Errorf("calling delegate: %w", err)
	}
...
func (b *Business) Delete(ctx context.Context, ur UserRole) error {
	...
	// Inform permissions, need to clear cache
	if err := b.del.Call(ctx, ActionDeletedData(ur.ID)); err != nil {
...
func (b *Business) QueryByUserID(ctx context.Context, userID uuid.UUID) (UserRole, error) {
	...
	if err != nil {
		return UserRole{}, fmt.Errorf("querying role: userID[%s]: %w", userID, err)
	}
