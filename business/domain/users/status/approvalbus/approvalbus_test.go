...
func insertSeedData(busDomain dbtest.BusDomain) (unitest.SeedData, error) {
	...
	as, err := approvalbus.TestSeedUserApprovalStatus(ctx, 10, busDomain.UserApprovalStatus)
	if err != nil {
...
func query(busdomain dbtest.BusDomain, sd unitest.SeedData) []unitest.Table {
	table := []unitest.Table{
		{
			Name: "query",
			ExpResp: []approvalbus.UserApprovalStatus{
			...
			ExcFunc: func(ctx context.Context) any {
				aprvlStatuses, err := busdomain.UserApprovalStatus.Query(ctx, approvalbus.QueryFilter{}, order.NewBy(approvalbus.OrderByName, order.DESC), page.MustParse("1", "5"))
				if err != nil {
					return err
				}
	...
	table := []unitest.Table{
		{
			...
			ExcFunc: func(ctx context.Context) any {
				aprvlStatus, err := busDomain.UserApprovalStatus.Create(ctx, approvalbus.NewUserApprovalStatus{
					Name:   "Test Approval Status",
					IconID: sd.UserApprovalStatus[0].IconID,
				})
				if err != nil {
			...
			CmpFunc: func(got any, exp any) string {
				gotResp, exists := got.(approvalbus.UserApprovalStatus)
				if !exists {
					return fmt.Sprintf("got is not an approval status %v", got)
				}

				expResp := exp.(approvalbus.UserApprovalStatus)
	...
	table := []unitest.Table{
		{
			...
			ExcFunc: func(ctx context.Context) any {
				aprvlStatus, err := busDomain.UserApprovalStatus.Update(ctx, sd.UserApprovalStatus[0], approvalbus.UpdateUserApprovalStatus{
