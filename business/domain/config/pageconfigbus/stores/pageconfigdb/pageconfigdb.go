...
func (s *Store) CreatePageTabConfig(ctx context.Context, config pageconfigbus.PageTabConfig) error {
	const q = `
	...
	if err := sqldb.NamedExecContext(ctx, s.log, s.db, q, config); err != nil {
		return fmt.Errorf("namedexeccontext: %w", err)
	}
...
func (s *Store) UpdatePageTabConfig(ctx context.Context, config pageconfigbus.PageTabConfig) error {
	const q = `
...
func (s *Store) DeletePageTabConfig(ctx context.Context, configID uuid.UUID) error {
	data := struct {
		ID uuid.UUID `db:"id"`
	}{
		ID: configID,
...
func (s *Store) QueryPageTabConfigByID(ctx context.Context, configID uuid.UUID) (pageconfigbus.PageTabConfig, error) {
	data := struct {
		ID uuid.UUID `db:"id"`
	}{
		ID: configID,
	...
	var config pageconfigbus.PageTabConfig
	if err := sqldb.NamedQueryStruct(ctx, s.log, s.db, q, data, &config); err != nil {
		if errors.Is(err, sqldb.ErrDBNotFound) {
			return pageconfigbus.PageTabConfig{}, fmt.Errorf("namedquerystruct: %w", pageconfigbus.ErrNotFound)
		}
		return pageconfigbus.PageTabConfig{}, fmt.Errorf("namedquerystruct: %w", err)
	}
...
func (s *Store) QueryPageTabConfigsByPageID(ctx context.Context, pageConfigID uuid.UUID) ([]pageconfigbus.PageTabConfig, error) {
	data := struct {
	...
	var configs []pageconfigbus.PageTabConfig
