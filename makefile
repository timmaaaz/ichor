...
ICHOR_APP       := ichor
AUTH_APP        := auth
BASE_IMAGE_NAME := localhost/superior
VERSION         := 0.0.1
...
ichor:
	docker build \
		-f zarf/docker/dockerfile.ichor \
		-t $(ICHOR_IMAGE) \
...
dev-logs-init:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(ICHOR_APP) -f --tail=100 -c init-migrate-seed

dev-describe-node:
	kubectl describe node

dev-describe-deployment:
	kubectl describe deployment --namespace=$(NAMESPACE) $(ICHOR_APP)

dev-describe-ichor:
	kubectl describe pod --namespace=$(NAMESPACE) -l app=$(ICHOR_APP)
...
dev-services-delete:
	kustomize build zarf/k8s/dev/ichor | kubectl delete -f -
...
dev-describe-replicaset:
	kubectl get rs
	kubectl describe rs --namespace=$(NAMESPACE) -l app=$(ICHOR_APP)
...
dev-shell:
	kubectl exec --namespace=$(NAMESPACE) -it $(shell kubectl get pods --namespace=$(NAMESPACE) | grep ichor | cut -c1-26) --container ichor-api -- /bin/sh
...
migrate:
	export ICHOR_DB_HOST=localhost; go run api/cmd/tooling/admin/main.go migrate
...
talk-logs-mem:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(ICHOR_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | grep "ms clock"
